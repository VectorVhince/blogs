    ...
    <div class="col-lg-8">
        <div class="panel panel-default bd-rad0 box-shadow">
            <div class="panel-body pd45">
                <div class="mgb40">
                    <span class="fs40">ARTWORK</span>
                </div>
                @if(!$artworks->isEmpty())
                @foreach($artworks as $artwork)
                <div class="row mgb20">
                    <div class="col-md-12">
                        <span class="dp-bl fs25 fc-red">{{ $artwork->title }}</span>
                        <span class="text-muted">Author: {{ $artwork->user }}</span> | <span class="text-muted">{{ date_format($artwork->created_at, 'F d Y') }}</span>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <img src="{{ asset('/img/uploads/' . $artwork->image) }}" class="img-responsive">
                    </div>
                    <div class="col-md-8">
                        <p>{{ substr($artwork->body,0,400) }}...  <a href="{{ route('artworks.show', $artwork->id) }}" class="fc-gold">See More</a></p>
                    </div>
                </div>
                <hr>
                @endforeach
