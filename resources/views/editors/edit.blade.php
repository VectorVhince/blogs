...
<div class="container">
    <div class="panel panel-default">
        <div class="panel-body text-center">
            <form action="{{ route('editors.update',$editors->id) }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('patch') }}
                <span class="fs40 fc-gold mgb5">Update your editors</span>
                <div class="form-group{{ $errors->has('editors_title') ? ' has-error' : '' }}">
                    <input type="text" name="editors_title" class="form-control mgb5" placeholder="Title" value="{{ $editors->editors_title }}" autofocus>
                ...
                <div class="form-group{{ $errors->has('editors_body') ? ' has-error' : '' }}">
                    <textarea name="editors_body" class="form-control mgb5" rows="10" placeholder="Content">{{ $editors->editors_body }}</textarea>
