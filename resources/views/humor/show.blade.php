...
@section('meta')
    ...
    <meta property="og:title"         content="{{ $humors->title }}" />
    <meta property="og:description"   content="{{ $humors->body }}" />
    <meta property="og:image"         content="{{ asset('/img/uploads/' . $humors->image) }}" />
@stop
...
<div class="container">
        ...
        <div class="col-lg-8">
            <div class="panel panel-default bd-rad0 box-shadow panel-bg">
                ...
                <div class="panel-body pdh45">
                    <div class="row mgb40">
                        <div class="col-md-10">
                            <span class="fs40">{{ $humors->title }}</span>
                            <div class="dp-bl">
                                <span class="text-muted">Author: </span>{{ $humors->user }} <span class="text-muted mgl10">Posted: </span>{{ date_format($humors->created_at, 'F d, Y') }}
                            </div>
                        </div>
                        <div class="col-md-2">
                        @if (Auth::user())
                            @if (Auth::user()->id == $humors->user_id || Auth::user()->role == 'superadmin')                                
                                <div class="dropdown" style="float: right;">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu1">
                                        ...
                                        @if(Auth::user()->role == 'superadmin')
                                          ...
                                          @endif
                                        @endif
                                        <li><a href="{{ route('humor.edit',$humors->id) }}"><img src="{{ asset('/img/edit.png') }}" class="ht20"> Edit</a></li>
                    ...
                    <div class="row">
                        <div class="col-lg-12 text-center">
                            <img src="{{ asset('/img/uploads/' . $humors->image) }}" class="img-responsive mgb40">
                        </div>
                    </div>
                    <div class="row mgb20">
                        <div class="col-lg-12">
              ...
              <div class="panel-body pd15">
                  ...
                  @if(!$stories->isEmpty())
                    @foreach($stories as $story)
                      <a href="{{ route('humor.show', $story->id) }}" class="fc-black dp-bl">
                        <div class="row mg0">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            ...
            <div style="height: 20px;" class="bgc-red mg0"></div>
                <div class="panel-body pd15">
                    <form action="{{ route('humor.comment',$humors->id) }}" method="post" enctype="multipart/form-data" id="formSubmit">
                        {{ csrf_field() }}
                        <div class="mgb20">
...
<div id="modal1" class="modal fade bs-example-modal-sm pdt200" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog modal-sm" role="document">
    ...
    <form action="{{ route('humor.destroy',$humors->id) }}" method="post">
    {{ csrf_field() }}
    {{ method_field('delete') }}
...
<div id="modal2" class="modal fade bs-example-modal-sm pdt200" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content text-center pd15">
      ...
      <div class="row mgt20">
          <a href="{{ route('humor.featured',[$humors->id]) }}"><button type="button" class="btn btn-success btn-sm">Yes</button></a>
          <button type="button" class="btn btn-primary btn-sm" data-dismiss="modal">No</button>
      </div>
...
<div id="modal3" class="modal fade bs-example-modal-sm pdt200" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content text-center pd15">
      ...
      <div class="row mgt20">
          <a href="{{ route('humor.unfeatured',[$humors->id]) }}"><button type="button" class="btn btn-danger btn-sm">Yes</button></a>
          <button type="button" class="btn btn-primary btn-sm" data-dismiss="modal">No</button>
      </div>
