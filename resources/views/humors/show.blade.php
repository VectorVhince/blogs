        ...
        <div class="panel panel-default bd-rad0 box-shadow">
            <div class="panel-body pd45">
                ...
                @if (Auth::user()->id == $humors->user_id)
                    <form action="{{ route('humors.destroy',$humors->id) }}" method="post">
                    {{ csrf_field() }}
                    {{ method_field('delete') }}
                        <ul class="list-inline">
                            <li class="pd0"><a href="{{ route('featured',[$humors->id,$humors->category]) }}"><img src="{{ asset('/img/featured.png') }}" class="ht60"></a></li>
                            <li class="pd0"><a href="{{ route('humors.edit',$humors->id) }}"><img src="{{ asset('/img/edit.png') }}" class="ht60"></a></li>
                ...
                <p>{{ $humors->body }}</p>
                <div>
                    <span class="text-muted">Author: {{ $humors->user }}</span> | <span class="text-muted">{{ date_format($humors->created_at, 'F d Y') }}</span>
                </div>
            </div>
        </div>
        <div class="panel panel-default bd-rad0 box-shadow mgt40">
            <div class="panel-body pd45">  
                <form action="{{ route('humors.comment',$humors->id) }}" method="post" enctype="multipart/form-data" id="formSubmit">
                    {{ csrf_field() }}
                    <div class="mgb40">
