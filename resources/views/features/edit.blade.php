    ...
    <div class="panel panel-default">
        <div class="panel-body text-center">
            <form action="{{ route('features.update',$features->id) }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('patch') }}
                <span class="fs40 fc-gold mgb5">Update your features</span>
                <div class="form-group{{ $errors->has('features_title') ? ' has-error' : '' }}">
                    <input type="text" name="features_title" class="form-control mgb5" placeholder="Title" value="{{ $features->features_title }}" autofocus>
                ...
                <div class="form-group{{ $errors->has('features_body') ? ' has-error' : '' }}">
                    <textarea name="features_body" class="form-control mgb5" rows="10" placeholder="Content">{{ $features->features_body }}</textarea>
