                ...
                <div class="panel-body pdh45">
                    <form action="{{ route('feature.update',$features->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                            <input type="text" name="title" class="form-control mgb20 bd-rad0 box-shadow" placeholder="Title" value="{{ $features->title }}">
                            @if ($errors->has('title'))
                                <span class="help-block"><strong>{{ $errors->first('title') }}</strong></span>
                            @endif
                        </div>
                        <div class="form-group{{ $errors->has('body') ? ' has-error' : '' }}">
                            <textarea name="body" class="form-control mgb20 bd-rad0 box-shadow ht500" placeholder="Content">{{ $features->body }}</textarea>
                            @if ($errors->has('body'))
